...
export const store = async (req, res) => {
  try {
    ...
    if (!name || !user_id) {
      return res.status(400).json({ message: 'Los campos name y user_id son requeridos', status: 400 });
    }
...
export const show = async (req, res) => {
  try {
    ...
    if (!isValidId(id)) {
      return res.status(400).json({ message: 'Identificador de categoría no válido', status: 400 });
    }
...
export const update = async (req, res) => {
  try {
    ...
    if (!name) {
      return res.status(400).json({ message: 'El nombre es obligatorio', status: 400 });
    }
